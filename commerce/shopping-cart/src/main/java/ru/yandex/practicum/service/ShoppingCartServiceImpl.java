...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    public ShoppingCartDto addProductToShoppingCart(String username, Map<UUID, Integer> products) {
        ...
        products.forEach((productId, quantity) ->
                oldProducts.merge(productId, quantity, Integer::sum));
    ...
    public ShoppingCartDto changeProductQuantity(String username, ChangeProductQuantityRequest request) {
        ...
        if (request.getNewQuantity() <= 0) {
            throw new NoProductsInShoppingCartException("Количество товара должно быть больше 0");
        }
        log.info("Запрашиваем актуальную корзину для пользователя {}", username);
        ShoppingCart cart = getOrCreateShoppingCart(username);
        checkCartIsActive(cart);
    ...
    private void validateProducts(Map<UUID, Integer> products) {
        if (products == null || products.isEmpty()) {
            throw new NoProductsInShoppingCartException("Список товаров не должен быть пустым");
        }
        if (products.values().stream().anyMatch(quantity -> quantity == null || quantity <= 0)) {
            throw new NoProductsInShoppingCartException("Количество каждого товара должно быть больше 0");
        }
    }
    ...
    public String getUsernameById(UUID cartId) {
        log.info("Получаем имя пользователя по id корзины {}", cartId);
